...
encounter_modified as (
    select
        ...
        format_timestamp('%T', e.start_time) As starting_time,
        format_timestamp('%T', e.stop_time) AS stoppage_time,
        timestamp_diff(e.stop_time, e.start_time, minute) AS encounter_duration_minutes,
        extract(year from e.start_time) AS encounter_year,
        format_timestamp('%b', e.start_time) AS encounter_month,
        format_timestamp('%A', e.start_time) AS encounter_day_of_week,
        e.encounterType,
        e.encounter_code,
    ...
    LEFT JOIN payers y ON e.payer_id = y.payer_id
